/*
  ...
  1. New Views
    - `users` view in public schema
      ...
      - Includes id, email, created_at, updated_at, metadata columns
      - Enables proper joins with user_documents and chat_sessions

  2. Security Functions
    - Admin-only access through SECURITY DEFINER functions
    - Email-based admin verification (@forwardassisthq.com)
    - Controlled access to user data and chat logs

  3. Admin Dashboard Functions
    - `get_all_user_documents()`: Returns all user documents with user info
